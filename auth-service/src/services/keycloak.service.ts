...
class KeycloakService {
    ...
    async createUser(username: string, password: string, email?: string, roles: string[] = []) {
        const token = await this.getAdminToken();

        try {
            const userEmail = email || `${username}@quizy-peazy.com`;
            ...
            if (!userId) {
                throw new ApiError('keycloak error', 'Failed to get user ID from Keycloak', 500);
            }
    ...
    private async assignRolesToUser(userId: string, roles: string[]): Promise<void> {
        const token = await this.getAdminToken();
        ...
        try {
            ...
            if (!client) {
                throw new Error(`Client '${Config.keycloak.clientId}' not found in Keycloak`);
            }
            ...
            const rolesToAssign = rolesResponse.data.filter((role: any) =>
                roles.includes(role.name)
            );

            if (rolesToAssign.length === 0) {
                console.warn(`No matching CLIENT roles found in Keycloak for: ${roles.join(', ')}`);
                return;
            }

            await axios.post(
                `${Config.keycloak.baseUrl}/admin/realms/${Config.keycloak.realm}/users/${userId}/role-mappings/clients/${clientUUID}`,
                rolesToAssign,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            console.log(`Assigned CLIENT roles [${roles.join(', ')}] to user ${userId}`);
        } catch (error: any) {
            console.error('Failed to assign roles:', error.response?.data || error.message);
            throw new ApiError('keycloak error', 'Failed to assign roles to user', 500);
        }
    }
    ...
    async deleteUser(userId: string): Promise<void> {
        ...
        try {
            ...
            if (error.response?.status === 404) {
                console.log(`User ${userId} already deleted from Keycloak`);
                return;
            }
            console.error('Failed to delete user from Keycloak:', error.response?.data || error.message);
            throw new ApiError('keycloak error', 'Failed to delete user from Keycloak', 500);
        }
    }
